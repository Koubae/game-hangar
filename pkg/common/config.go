...
func NewConfig(logger Logger) *Config {
	...
	appName := GetEnvString("APP_NAME", "unknown")
	appVersion := GetEnvString("APP_VERSION", "0.0.1-dev")
	appCommitID := GetEnvString("APP_COMMIT_ID", "")
	env := Environment(GetEnvString("APP_ENV", string(EnvDev)))
	...
	serverReadTimeout := GetEnvInt("APP_SERVER_READ_TIMEOUT_SECONDS", 15)
	serverWriteTimeout := GetEnvInt("APP_SERVER_WRITE_TIMEOUT_SECONDS", 15)
	serverIdleTimeout := GetEnvInt("APP_SERVER_IDLE_TIMEOUT_SECONDS", 60)
	serverShutdownGraceTimeout := GetEnvInt("APP_SERVER_SHUTDOWN_GRACE_TIMEOUT_SECONDS", 10)
	serverMaxHeaderBytes := GetEnvInt("APP_SERVER_MAX_HEADER_BYTES", 8192)

	logLevel := LogLevel(GetEnvString("APP_LOG_LEVEL", string(LogLevelInfo)))
	...
	logFilePath := GetEnvString("APP_LOG_FILE", "logs/app.log")
